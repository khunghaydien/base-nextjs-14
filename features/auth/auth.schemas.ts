...
export const createEmailSchema = (t: (key: string) => string) =>
  z.string().min(1, t("email_required")).email(t("email_invalid"));

// Shared password schema
export const createPasswordSchema = (t: (key: string) => string) =>
  z.string().min(1, t("password_required")).min(6, t("password_min_length"));
...
export const createConfirmPasswordValidation = (
  ...
  z
    ...
    .refine(
      (data) => {
        const password =
          fieldName === "confirmPassword" ? data.password : data.newPassword;
