...
import { signInWithGoogle, signInWithFacebook, clearError, clearAuthUrl, clearAll } from "./social-sign-in.slice";
...
export function useSocialAuth() {
  ...
  const { isLoading, error, authUrl } = useAppSelector((state) => state.socialSignIn);
  ...
  const handleSignInWithProvider = async (provider: SocialProvider) => {
    ...
    if (result && (signInWithGoogle.fulfilled.match(result) || signInWithFacebook.fulfilled.match(result))) {
